...
export default function AdminTeachers() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [t, s] = await Promise.all([getAllTeachers(), getSubjects()]);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (message.includes("EMAIL_EXISTS")) {
        ...
        message = "Permission denied. Check Firestore rules allow admin writes.";
      } else if (message.includes("Unauthorized")) {
        message = "You must be logged in as an admin to create teachers.";
      }
  ...
  async function handleDelete(uid, name) {
    if (
      !window.confirm(
        `Remove teacher ${name}? This removes their profile but NOT their login account.`
      )
    )
      return;
              ...
              <div key={t.uid} className="card">
                <div className="card-body">
                    ...
                    >
                      {(t.classes || []).length ? (
                        t.classes.map((c) => (
                          ...
                          >
                            {CLASSES.find((x) => x.code === c)?.label || c}
                          </span>
                        ))
                      ) : (
                        <span
                  ...
                  {hasWarning && (
                    ...
                    >
                      ⚠ This teacher will see a blank dashboard until you
                      assign
                      {missingCls && missingSub
