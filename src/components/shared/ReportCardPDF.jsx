...
function ReportCardDocument({ student, classLabel, term, year, subjects, allMarks, studentList, vacDate, nextTermDate }) {
  const studentMarks = {};
  ...
  const totals = subjects.map(s => calcWeightedTotal(studentMarks[s.code])).filter(t => t !== "");
  const overallAvg = totals.length ? Math.round(totals.reduce((a,b)=>a+b,0)/totals.length*10)/10 : null;

  // Class position (by overall avg)
  const allAvgs = studentList.map(st => {
    const tots = subjects.map(s => calcWeightedTotal(allMarks[s.code]?.[st.id]||{})).filter(t=>t!=="");
    ...
    <Document title={`Report Card — ${student.name}`}>
      <Page size="A4" style={S.page}>
        ...
        {subjects.map((subj, idx) => {
          ...
          const subjTotals = studentList.map(st => calcWeightedTotal(allMarks[subj.code]?.[st.id]||{})).filter(t=>t!=="");
          const subjAvg    = subjTotals.length ? Math.round(subjTotals.reduce((a,b)=>a+b,0)/subjTotals.length*10)/10 : null;
          ...
          return (
            <View key={subj.code} style={rowStyle}>
              ...
              <Text style={{ ...S.cell, flex: CW.total, textAlign: "center", fontFamily: "Helvetica-Bold",
              ...
              <Text style={{ ...S.gradeBadge, flex: CW.grade, textAlign: "center",
              ...
              <Text style={{ ...S.cell, flex: CW.level, color: grade ? grade.color : "#5A7A68", fontSize: 7.5 }}>
        ...
        <View style={{ marginTop: 8, backgroundColor: "#E8F5EE", padding: "4 8", borderRadius: 3 }}>
          <Text style={{ fontSize: 7, color: "#1A6B45", textAlign: "center" }}>
            Ghana Education Service · {SCHOOL_NAME} · {SCHOOL_CIRCUIT} Circuit
          </Text>
        </View>
      </Page>
    </Document>
...
export function ReportCardDownloadBtn({ student, classLabel, term, year, subjects, allMarks, studentList, vacDate, nextTermDate }) {
  return (
    <PDFDownloadLink
      document={
        <ReportCardDocument
          student={student} classLabel={classLabel}
          term={term} year={year} subjects={subjects}
          allMarks={allMarks} studentList={studentList}
          vacDate={vacDate} nextTermDate={nextTermDate}
        />
      }
